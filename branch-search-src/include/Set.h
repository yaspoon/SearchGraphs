...
class Set
{
    public:
        ...
        Set<T> operator-(Path inPath)
        {
            Set<T> tmp;
            for(Iterator<T> iT = currentSet.begin(); iT != currentSet.end(); ++iT)
            {
                if(inPath != iT.getData())
                {
                    tmp.insert(iT.getData());
                }
            }
        ...
        T smallest()
        {
            ...
            if(size() != 0)
            {
                ...
                small = currentSet.begin().getData(); //By default set the smallest to the start of the list
